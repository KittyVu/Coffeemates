...
export default function PostCard({ post }: { post: PostType }) {
  ...
  const ownerImg = user?.photoURL ? `${user.photoURL}` : '';
  const [userImg, setUserImg] = useState(ownerImg);
  ...
  // fetch photoURL of post owner
  useEffect(() => {
    if (!user) return;

    const fetchUserPhoto = async () => {
      try {
        const res = await fetch(`${apiUrl}/api/user/${post.uid}`);
        const data = await res.json(); console.log("data after fetching user: ", data)
